...
The function `mapdims()` maps over both dimensions of the 2D Array and return as an Array of Arrays. First element contains the row-wise results; the second holds the column-wise computations. The functions `mapc()` and `mapr()` call `mapslices()` to execute operations column-wise or row-wise respectively.
