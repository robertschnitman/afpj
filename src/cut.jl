"""
cut(x, g) # array, group variable (position number).
...
cut(A, 1) # 3-element Array{Array{Int32,2},1}

# Source
Main - https://github.com/robertschnitman/afpj/blob/master/src/cut.jl
"""

cut(x, g) = map(i -> x[x[:, g] .== i, :], unique(x[:, g]))